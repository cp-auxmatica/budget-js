...
import { getFirestore, doc, getDoc, setDoc, updateDoc, deleteDoc, onSnapshot, collection, query, getDocs, addDoc, runTransaction, setLogLevel } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
async function initApp() {
    ...
    // Set up initial view
    window.showView('dashboardView', document.querySelector('[onclick*="dashboardView"]'));
...
// --- CORE FUNCTIONS (REPLACED INDEXEDDB WITH FIRESTORE) ---
function getCollection(path) {
    const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
    // Using a simple path for now, but keeping the app_id for future multi-user enhancements.
...
window.startEditCategory = async function(button, categoryId) {
...
window.deleteSubcategory = async function(categoryId, subcategoryName) {
...
window.startEditSubcategory = async function(button, categoryId, oldName) {
...
window.editGroceryShoppingList = async function(listId) {
...
window.deleteGroceryShoppingList = async function(listId) {
...
window.toggleSubscriptionStatus = async function(id, currentStatus) {
...
window.toggleBudgetPaidStatus = async function(id) {
    const docRef = doc(getCollection('budgets'), id);
...
window.deleteItemizedEntry = async function(expenseId, itemIndex) {
    const docRef = doc(getCollection('expenses'), expenseId);
    const docSnap = await getDoc(docRef);
